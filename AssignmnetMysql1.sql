...
# 2. Create a table called countries with the following columns name, population, capital
...
INSERT INTO countries 
VALUES('Costa Rico',1272,'San José'),
('Jamaica',1126,'Kingston'),
('Papua New Guinea States',224,"Port Moresby"),
('Nicaragua',230,'Managua'),
('Poland',109,'Warsaw')
...
CREATE TABLE suppliers(supplier_id INT PRIMARY KEY, supplier_name VARCHAR(100), location VARCHAR(100));

CREATE TABLE product(product_id INT PRIMARY KEY, product_name  VARCHAR(100) UNIQUE NOT NULL, description_ VARCHAR(200), 
					supplier_id INT, FOREIGN KEY(supplier_id) REFERENCES suppliers(supplier_id)
                    ON DELETE CASCADE
                    ON UPDATE CASCADE);
                    	
CREATE TABLE assignment.stock(id INT PRIMARY KEY, product_id INT, FOREIGN KEY(product_id) REFERENCES assignment.product(product_id)
					ON DELETE CASCADE
                    ON UPDATE CASCADE,
                    balance_stock INT);
...
VALUES(101,'Apple','Think different',1),
	   (102,'Dollar Shave Club','Shave Time Shave Money',2),
       (103,'Loreal',"Because You're Worth It",3),
       (104,'Bounty','The Quicker Picker Upper',4);
...
ALTER TABLE emp ADD deptno INT;emp_sal
...
CREATE UNIQUE INDEX id ON emp(emp_id);
